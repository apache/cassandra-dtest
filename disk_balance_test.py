...
from tools import since, new_node, create_c1c2_table, insert_c1c2, query_c1c2, no_vnodes
...
class TestDiskBalance(Tester):
    ...
    def disk_balance_stress_test(self):
        ...
        node1.stress(['write', 'n=10000', '-rate', 'threads=100', '-schema', 'replication(factor=2)'])
        cluster.flush()
    ...
    def disk_balance_bootstrap_test(self):
        ...
        if DISABLE_VNODES:
            for node in cluster.nodelist():
                node.nodetool('rebalancedisks')
            self.assertTrue(len(node5.grep_log("No sstables to REBALANCE for keyspace1.standard1")) > 0)
    ...
    def alter_replication_factor_test(self):
        ...
        node1.stress(['write', 'n=100000', '-rate', 'threads=100'])
        cluster.flush()
        for node in cluster.nodelist():
            self.assert_balanced(node)
    ...
    def assert_balanced(self, node):
        ...
        for sstabledir in node.get_sstables_per_data_directory('keyspace1', 'standard1'):
            ...
            sums.append(sum)
        assert_almost_equal(*sums, error=0.2)
